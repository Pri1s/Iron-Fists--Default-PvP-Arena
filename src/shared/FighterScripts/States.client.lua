...
UserInputService.InputBegan:Connect(function(input, gameProcessedEvent)
    ...
    if input.UserInputType == Enum.UserInputType.MouseButton1 or input.KeyCode == Enum.KeyCode.ButtonX then
        if VitalsUi.Stamina.Offensive.Bar.Size.X.Scale -
            (stateEngine.Attributes.ofStaminaDrain / stateEngine.Attributes.ofStamina) <= 0 then
            return
        end
    ...
    elseif input.UserInputType == Enum.UserInputType.MouseButton2 or input.KeyCode == Enum.KeyCode.ButtonL2 then
        if VitalsUi.Stamina.Block.BarSlot.Bar.Size.X.Scale -
            (stateEngine.Attributes.dvStaminaDrain / stateEngine.Attributes.dvStamina) <= 0 then
            return
        end
    ...
    elseif input.KeyCode == Enum.KeyCode.F or input.KeyCode == Enum.KeyCode.ButtonY then
        ...
        if tick() - Cooldowns.Uppercut.previousInputTime < Cooldowns.Uppercut.Duration then
            return
        end
    ...
    elseif input.KeyCode == Enum.KeyCode.Space or input.KeyCode == Enum.KeyCode.ButtonL1 then
        if stateEngine:GetTarget() == "Head" then
            return
        end
